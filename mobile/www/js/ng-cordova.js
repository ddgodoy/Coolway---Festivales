...
// install noiseMeter
// repo in local storage

angular.module('ngCordova.plugins.noiseMeter', [])

  .factory('$cordovaNoiseMeter', ['$q', function ($q) {

    return {
      getNoise: function () {
        var q = $q.defer();

        NoiseMeter.getNoise(function (result) {
          q.notify(result);
        }, function (err) {
          q.reject(err);
        });
            ...
            if(window.cordova) {
                ...
                if(cordovaMetadata.hasOwnProperty("org.apache.cordova.inappbrowser") === true) {
                    if(typeof jsSHA !== "undefined") {
                        ...
                        var signatureObj = $cordovaOauthUtility.createSignature("POST", "https://api.twitter.com/oauth/request_token", oauthObject,  { oauth_callback: "http://twitter.com" }, clientSecret);
                        $http({
                            method: "post",
                            ...
                            data: "oauth_callback=" + encodeURIComponent("http://twitter.com")
                        })
                            .success(function(requestTokenResult) {
                                ...
                                browserRef.addEventListener('loadstart', function(event) {
                                    if((event.url).indexOf("http://twitter.com") === 0) {
                                        var callbackResponse = (event.url).split("?")[1];
                                        var responseParameters = (callbackResponse).split("&");
                                        var parameterMap = {};
                                        for(var i = 0; i < responseParameters.length; i++) {
